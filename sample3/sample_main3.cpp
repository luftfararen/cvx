...
void main()
{
  ...
  auto fast_irounded = fast_iround(src);
  std::cout << std::fixed << std::setprecision(2) << "iround(" << src << "f) == " << irounded
            << "\n"
            // iround(8388609.00f) == 8388610

            << "lround(" << src << "f) == " << lrounded << "\n"
  << "fast_iround(" << src << "f) == " << fast_irounded << "\n";
